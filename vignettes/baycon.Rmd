...
In this first outline, we will just recapture some of Paul's investigations to see how the `baycon` function is doing. We will be using already generated simulated data (that lives in `data`). 
...
out <- baycon(bulkExpression = p_bulkExpressionSimMat, mle = F, 
              chains = 3, iter = 2000, init = 0, verbose = F, refresh = 0, 
...
abline(0,1)
cor(propMat[,1], p_propInv)
```


```{r mle, eval=F, echo=F}
mvnPropEsts <- out$mvn$mean
...
plot(mvnPropEsts[,2], p_propInv)
abline(coef = coef(lm(mvnPropEsts[,2] ~ p_propInv)))
summary(lm(mvnPropEsts[,2] ~ p_propInv))
```


> Lingering QUESTIONs: Are the error estimates actually accurate!!!???? How to test?  
